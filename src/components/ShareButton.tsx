...
export function ShareButton({ movies, userName = 'Someone' }: ShareButtonProps) {
  ...
  const generateShareText = () => {
    ...
    let shareText = `🎬 Check out ${userName}'s CineVault!\n\n`
    ...
    shareText += `\n🔗 View the full CineVault: ${window.location.origin}`
  ...
  const shareViaEmail = () => {
    const subject = encodeURIComponent(`${userName}'s CineVault`)
      ...
      {showShareMenu && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 w-64 bg-gray-800 border border-gray-700 rounded-xl shadow-xl z-50 p-4">
            <h3 className="text-white font-semibold mb-3">Share Your CineVault</h3>
